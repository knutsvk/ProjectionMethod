...
In order to investigate how the impusively started system evolves to
...
\begin{figure*}[t]
    ...
    \caption
    {
        Transient results for ${\rm Re}=200$ at different times: $t=0.04$
...
\begin{figure*}[t]
    ...
    \caption
    {
        First component $u$ of velocity through $x=0.5$. The filled black
        diamonds corresponds to tabulated results from Ghia {\em et al.\ }
...
\begin{figure*}[t]
    ...
    \caption
    {
        Second component $v$ of velocity through $y=0.5$. The filled black
        diamonds corresponds to tabulated results from Ghia {\em et al.\ }
        \cite{ghia1982high}. \textcolor{red}{$\times$} $N=17$,
        \textcolor{blue}{$\circ$} $N=33$, \textcolor{teal}{$\triangle$} $N=65$,
        \textcolor{purple}{\huge $_{\cdot}$} $N=129$. 
...
{\em et al.}, we see that the streamline pattern is near identical for all
...
the distribution of flow. Figure \ref{fig:vorticity} depicts the vorticity
...
\begin{itemize}
        \item Fix plots for $\psi$ and $\omega$: rotate ylabel, check range for
            meta values, compare with transient
\end{itemize}
...
\begin{itemize}
    \item Could obviously implement Bingham plastic.
    \item This could also be done for Re>0. 
