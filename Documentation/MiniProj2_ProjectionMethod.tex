...
\begin{itemize}
    \item {\bf write something general}
\end{itemize}
...
\begin{itemize}
    \item {\bf Comment on values of $\psi$, $\omega$ and $\bm{u}$? Colorbars?}
    \item {\bf Expand captions for $\psi$- and $\omega$-results}
\end{itemize}
...
\begin{itemize}
    \item {\bf Runtime (and no of time steps) as a function of Re and $N$}
    \item {\bf Runtime per iteration? Should be $O(N^4)$, right? }
\end{itemize}
...
computational effort is in solving the linear systems of equations. We could
...
\begin{itemize}
    \item {\bf Expand on Hockney algorithm?}
    \item {\bf The decoupled systems are diagonal and solved in $O(N)$}
    \item {\bf Solution of the decoupled systems can be parallelized!}
\end{itemize}
